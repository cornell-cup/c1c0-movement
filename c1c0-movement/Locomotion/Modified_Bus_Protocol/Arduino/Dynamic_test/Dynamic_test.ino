...
void test1(){
      if(Serial1.available() > 0) //checks if there is data in the serial buffer to be read
      {
        ...
        r2p_dynamic_decode1(recv_buffer,address,l1,&checksum,type, &data_len, &t); // decoding received data
...
void test2(){
  while(1){
      ...
      {
        uint32_t l1 = R2P_HEADER_SIZE - 3;
        uint8_t * recv_buffer = (uint8_t *)malloc(sizeof(uint8_t) * l1);
        Serial1.readBytes(recv_buffer,l1); // reads the buffer data storing a buffer_len length of data in in recv_buffer
        uint32_t index = r2p_dynamic_decode1(recv_buffer,address,l1,&checksum,type, &data_len, &t); // decoding received data
        free(recv_buffer);
        if(index < 0)
        ...
        buffer_len = data_len + R2P_HEADER_SIZE;
        data = (uint8_t *)malloc(sizeof(uint8_t) * data_len);
        r2p_dynamic_decode2(recv_buffer2,data,&data_len, index,&checksum);
        printmsg();
        free(recv_buffer2);
    ...
    if(Serial1.available() > 0) //checks if there is data in the serial buffer to be read
      {
        uint8_t buff[50];
        buffer_len = 50;
        Serial1.readBytes(buff,buffer_len); // reads the buffer data storing a buffer_len length of data in in recv_buffer
        r2p_decode(buff,address,buffer_len,&checksum,type,data, &data_len, &t); // decoding received data
        uint8_t * send_buffer = (uint8_t *)malloc(sizeof(uint8_t) * (data_len + R2P_HEADER_SIZE));
