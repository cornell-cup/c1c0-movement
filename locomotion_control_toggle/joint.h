...
{
    //Serial.println("why arent we here");
    uint16_t currentPosition; // 16-bit response from encoder
    bool binaryArray[16];     // after receiving the position we will populate this array and use it for calculating the checksum
    // get first byte which is the high byte, shift it 8 bits. don't release line for the first byte
    //Serial.println("yassssss");
    ...
    //Serial.println("this shi bussin");      
}

// TODO angle should be uint_8 when implementing jetson
inline void setTargetAngle(encoder_t *encoder, int angle) {
  if (angle < (encoder->min_angle)) {
    //Serial.println("MINNED");
    encoder->target_angle = encoder->min_angle;
  }
  else if (angle > (encoder->max_angle)) {
    //Serial.println("MAXED");
    encoder->target_angle = encoder->max_angle;
  }
  else{
